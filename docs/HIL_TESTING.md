...
- **Explicit Fixture Isolation (`flashed_rtt`, `ensure_target_flashed` in `tools/conftest.py`):** Ensures target binaries are flashed as needed per test class, enabling isolated single-test runs (e.g. `pytest -k test_ts703_rtt_injection`) without depending on prior test execution sequence.
...
- **Atomic Execution Fixtures (`target_reset_run` in `tools/conftest.py`):** Ensures CPU state (halt, breakpoints, execution registers) is isolated per test case and cleanly restored on teardown, enabling reliable single-test execution (`pytest -k test_ts403_single_step`).
